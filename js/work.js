...
<html lang="en">
  ...
  <style>
/* 1. 2. blue eye & remove blue eye만들기
  3초 transition, fill 속성 사용하기
*/
.eye-color {
...
/* 3. 4. happy, sad  만들기
  rotate 180도로 입을 뒤집기, 너무 크면 scale로 줄이기
  svg 벡터그래픽은 왼쪽 상단 0,0이 transform의 중심점이라는것을 이해하기.
  transform-origin을 입의 중심으로 이동하기
*/
.sad .mouth {

}

/*  5. 6 고양이 움직이기 animation
    눈동자 움직이기
    moveEyes를 채우자. translate로 V자로 움직이도록 해보자.
 */
...
.move-around .left-whiskers {
  animation: leftWhiskers 2s infinite;
  transform-origin: 350px 325px;
}
...
.move-around .right-whiskers {
  animation: rightWhiskers 2s infinite;
  transform-origin: 350px 325px;
}

/*  5. 6 고양이 움직이기 animation
  고양이 전체 움직이기
  X축 -10에서 15정도, Y축 -10에서 25정도 움직여보자
*/
.move-around {
...
$(document).ready(function () {
  $(document).on("click", "#1", function () {
    $(".cat").addClass("blue-eyes");
  });
  $(document).on("click", "#2", function () {
    $(".cat").removeClass("blue-eyes");
  });
  ...
  $(document).on("click", "#5", function () {
    $(".cat").addClass("move-around");
  });
  $(document).on("click", "#6", function () {
    $(".cat").removeClass("move-around");
  });
});
</script>
</head>
<body>
<div>
<button id="1">blue eye</button>
<button id="2">remove blue eye</button>
<button id="3">happy</button>
  <button id="4">sad</button>
  <button id="5">move around</button>
<button id="6">sit</button>
  </div>
